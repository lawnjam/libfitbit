...
import sys, os, serial, threading, time, operator, struct, datetime
...
##                print datetime.datetime.now().second, datetime.datetime.now().microsecond, "<<", self.ant.decodeMessage(data)
                self.ant.decodeMessage(data)
            else:
